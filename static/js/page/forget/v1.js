(function($){
	...
	$(function(){
		...
		doReset.on('click', function(event) {
			event.preventDefault();
			getVals();
			...
			.done(function(data) {
				...
				if(result){
					var dialog = $.dialog({
							minWidth:1000,
							html: '<div style="height:45px;width:270px;text-align:center;line-height:45px;background:rgba(0,0,0,0.77);color:#fff;">密码修改成功，正在跳转到登录页面！</div>',
			              	timers:[
				                {
									callback: function(){
										this.hide();
